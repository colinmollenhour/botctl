...
1. **TUI-backed one-shot execution**
   - `prompt` launches Claude in a new detached tmux session, waits for `ChatReady`, submits via the same guarded external-editor/keybinding handoff, waits for the reply, and prints only assistant text to stdout.
   - Small inputs are submitted directly through pending prompt storage; inputs larger than `--large-prompt-threshold` are written to a state-dir instruction file and Claude receives a short prompt pointing at that file.
...
3. **One-shot submission to an existing pane**
...
For large combined prompts, it creates `<state-dir>/prompt-runs/.../instructions.md` and submits a short file-pointer prompt through the normal handoff path. Unless `--keep-temp` is set, the temp instruction file is removed after a successful response; with `--keep-temp`, the path is reported on stderr for debugging.
