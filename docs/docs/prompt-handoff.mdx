...
1. **TUI-backed one-shot execution**
   - `prompt` launches Claude in a new detached tmux session, waits for `ChatReady`, pastes the resolved prompt into the interactive TUI through tmux, waits for the reply, and prints only assistant text to stdout.
