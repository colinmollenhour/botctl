...
1. Start `runtime` once.
2. Open `dashboard` to see Claude Code panes, screen-detected Codex CLI panes, passively resolvable OpenCode panes, and Pi panes.
3. Use `yolo` on the pane or workspace you want babysat.
4. Use `serve` when you need a live event stream or machine-readable observation.
5. Use `last-message` when the visible pane excerpt is not enough.
...
`runtime` is the single owner of live pane observation, shared yolo supervision, and guarded actions. The other long-lived commands are clients of that runtime.
