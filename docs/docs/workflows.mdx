...
## Recover a missing Claude or Grok session
...
Recovery offers are Claude and Grok only. Botctl must previously have checkpointed a verified Claude or Grok pane with a valid provider session UUID. A `Crashed` row means a later successful stable all-pane inventory did not contain that original pane object. It is conservative evidence of absence, not an inference that a human or crash closed the pane. An inventory failure, malformed server identity, or unstable server identity creates no recovery record.
...
3. Review the original identity, provider session UUID, exact resume command (`cd <cwd> && claude --resume <uuid>` or `cd <cwd> && grok --resume <uuid>`), target, and lifecycle in the details panel.
