...
`prompt` launches a new detached tmux session, waits for `ChatReady`, submits through the existing external-editor/keybinding handoff, handles only the same safe survey/folder-trust/permission blockers as YOLO unless `--no-yolo` is set, waits for the final assistant reply, and prints only assistant text to stdout. Use `submit-prompt` instead when you already have an existing pane you want to target.
