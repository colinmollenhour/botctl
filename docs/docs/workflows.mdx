...
## Recover a missing Claude session

Use this only after another tool recreates the tmux server, sessions, windows, and shell panes. Botctl does not recreate tmux objects, restore layouts or scrollback, launch Claude automatically, clear shell input, or press Enter.

Recovery offers are Claude-only. Botctl must previously have checkpointed a verified Claude pane with a valid Claude session UUID. A `Crashed` row means a later successful stable all-pane inventory did not contain that original pane object. It is conservative evidence of absence, not an inference that a human or crash closed the pane. An inventory failure, malformed server identity, or unstable server identity creates no recovery record.
...
3. Review the original identity, Claude UUID, exact `cd <cwd> && claude --resume <uuid>` command, target, and lifecycle in the details panel.
