...
- recovery rows are Claude and Grok only and preview the exact `cd <cwd> && claude --resume <uuid>` or `cd <cwd> && grok --resume <uuid>` command, original identity, lifecycle, target, or precise disabled reason
