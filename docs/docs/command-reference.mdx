...
Purpose: start the central local runtime that owns live observation, guarded actions, yolo supervision, and durable verified-Claude recovery checkpoints.
...
- each runtime invocation journals its run and checkpoints only verified Claude panes with valid Claude session UUIDs
- a graceful stop retires that run's outstanding checkpoints; an abandoned run is reconciled only after a later successful stable all-pane tmux inventory
- a `Crashed` recovery means the checkpointed original pane object was absent from that inventory; it does not infer whether the pane disappeared intentionally
- failed, malformed, or unstable inventories create no crash record and make no recovery matching decision
...
Purpose: open the runtime-backed live TUI for supported agent panes and conservative Claude-only recovery offers after an external tool recreates missing tmux shell panes.
...
- recovery rows are Claude-only and preview the exact `cd <cwd> && claude --resume <uuid>` command, original identity, lifecycle, target, or precise disabled reason
...
Purpose: open the live dashboard TUI for supported agent panes and Claude-only recovery offers after an external tool recreates missing tmux shell panes.
...
- recovery rows are not live classifier panes and never receive pane YOLO or unstick actions
- exact same-object or logical coordinates-plus-cwd matching must identify one compatible shell target; fuzzy, cwd-only, missing, ambiguous, and conflicting candidates remain disabled
- lowercase `r` refreshes, uppercase `R` stages one ready `Crashed` recovery without Enter, uppercase `D` dismisses eligible records, and `Enter` only navigates
- inspect the exact displayed command and target after staging, then press Enter yourself; botctl never submits it or automatically retries `Staged`/`Uncertain` records
