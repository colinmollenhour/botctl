...
- `dashboard`, `yolo`, and `serve` connect to this process instead of running independent long-lived control loops
...
Start syntax:
```bash
botctl yolo [start] (--pane %ID|session:window.pane | --all) [--follow] [--poll-ms N] [--format human|jsonl] [--live-preview] [--state-dir PATH] [--workspace PATH|UUID]
```
...
- shows current classified state, foreground agent uptime, wait time, branch, workspace grouping, pane PID, process-tree average CPU, memory, and observed active `Cook` time
