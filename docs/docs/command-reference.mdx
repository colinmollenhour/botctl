...
`botctl` supports Claude Code as an actively guarded automation target and OpenCode as a passively discovered dashboard target. OpenCode panes are visible in the dashboard when their tmux title and cwd can be matched safely to OpenCode's SQLite session database; keypress automation remains Claude-only.
...
- Usage errors exit with code `2` and point at focused help instead of dumping global help.
...
Purpose: open the live TUI for Claude Code panes and passively resolvable OpenCode panes, grouped by workspace, with per-pane and per-workspace YOLO controls for Claude.
...
- OpenCode support is visibility-only; YOLO and guarded automation remain Claude-only
...
Syntax:
```bash
botctl doctor [--session NAME] [--pane %ID|session:window.pane] [--history-lines N] [--bindings-path PATH] [--json]
```
...
Purpose: open the live dashboard TUI for Claude Code panes, passively resolvable OpenCode panes, and Claude YOLO toggles.
...
Targeting: no explicit pane target; scans all Claude Code panes and passively resolvable OpenCode panes.
...
Targeting:
- `start` can target one pane or scan all Claude Code panes with `--all`
