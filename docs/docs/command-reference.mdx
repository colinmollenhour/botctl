...
Purpose: run the small MCP-compatible JSON-RPC server for persistent agent sessions backed by managed tmux windows. Claude is the default provider; Codex and Agy are also supported on `spawn`.
...
Tools (clients typically expose them under the server name, e.g. `mcp__botctl__spawn`):

- `spawn` (optional `provider`: `claude` default, `codex`, `agy`; optional `model`, `effort`, `agent` — validated per provider)
- `prompt`
- `wait`
- `kill`
- `snapshot`
