...
- `prompt` success output is assistant text only on stdout; launch/wait/temp-file progress goes to stderr.
...
Purpose: run a one-shot Claude request through an observable interactive Claude TUI in a new detached tmux session. It does not call `claude -p` or `claude --prompt`.
...
- `--session NAME` defaults to a generated `botctl-prompt-*` session.
- `--window NAME` defaults to `claude`.
...
- progress, temp paths, cleanup warnings, and wait status go to stderr.
- the tmux session is left running for inspection.
- submission waits for `ChatReady`, uses the same keybinding/external-editor handoff as `submit-prompt`, waits for final `ChatReady`, and refuses to print stale transcript output.
...
Purpose: open the live TUI for Claude Code panes, screen-detected Codex CLI panes, passively resolvable OpenCode panes, and Pi panes, grouped by workspace, with per-pane and per-workspace YOLO controls for Claude and Codex.
...
- shows current classified state, foreground agent uptime, wait time, branch, and workspace grouping
