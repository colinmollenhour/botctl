...
- other user-facing long-lived commands connect to this process instead of running their own control loops
...
Purpose: open the live TUI for Claude panes, grouped by workspace, with per-pane and per-workspace YOLO controls backed by shared runtime state.
...
- queries the central runtime for Claude panes
