...
Purpose: run the small MCP-compatible JSON-RPC server for persistent agent sessions backed by managed tmux windows. Claude is the default provider; Codex, OpenCode, and Pi are also supported on `spawn`.
...
- `spawn` (accepts optional `provider`: `claude` default, `codex`, `opencode`, `pi`)
- `prompt`
- `wait`
- `kill`
- `snapshot`
