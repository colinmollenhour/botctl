...
Purpose: open the live TUI for Claude panes and passively resolvable OpenCode panes, grouped by workspace, with per-pane and per-workspace YOLO controls for Claude.
...
Purpose: open the live dashboard TUI for Claude panes and YOLO toggles.
...
Targeting: no explicit pane target; scans all Claude panes.
