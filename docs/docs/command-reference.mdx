...
- disables tmux `status` in botctl-created hidden sessions so user statusline commands do not run there
- `stop` shuts down the current runtime
- `--foreground` runs the runtime in the current process instead of the hidden tmux session
- `--reconcile-ms N` (default: `1000`) is the maximum delay before a dirty pane receives a fresh targeted reconcile; values above 10 seconds also set the full-state safety interval
...
- `--ready-timeout-ms` default `30000`; it bounds the initial wait for `ChatReady`.
- `--idle-timeout-ms` default `600000`; despite the flag name, it is a fixed response deadline that starts after verified prompt submission and does not reset on progress.
- `--agent megamind` with an idle timeout below `3600000` prints a warning to stderr but does not change the configured timeout. Use `28800000` for an eight-hour run.
...
- `--verbose` prints launch, wait, and temp-file progress to stderr.
- Safe YOLO handling is enabled by default. The separate `yolo` command accepts bare `--follow`; omit it to avoid tailing events. The flag accepts no value.
- `-- CLAUDE_ARG ...` passes additional arguments to the interactive Claude command. Prompt/headless mode (`-p`/`--prompt`) is still refused.

Output and safety:

- stdout contains only the fresh final assistant message text on success.
- with `--verbose`, progress, temp paths, and wait status go to stderr; cleanup and residual-composer warnings always go to stderr.
- after a fresh terminal assistant record, a stable empty composer completes normally; botctl kills the captured prompt window only when its captured pane remains the sole exact pane, otherwise it warns and retains the window.
- after a fresh terminal assistant record, two stable observations of an empty composer plus `ghost_shell=confirmed` and no other busy evidence also complete successfully; botctl warns about the stale footer and retains the window.
- after a fresh terminal assistant record, stable occupied composer text also completes successfully: botctl prints the final assistant text, warns with `botctl submit-composer --pane %ID` and `botctl clear-composer --pane %ID`, and retains the window.
- failed prompt windows are left alive for inspection.
- when `prompt` creates the owning tmux session, it disables tmux `status` only for that session; existing user-provided sessions are not changed.
- submission waits for `ChatReady`, proves the pasted text is bot-owned, verifies that Claude consumed it, and refuses stale transcript output.
...
- Codex YOLO can approve command permission dialogs by sending `y` for `Yes, proceed`; OpenCode, Pi, Grok, and Antigravity support is visibility-only (plus `last-message` where applicable), and broader guarded automation remains Claude-only
...
- consumes authoritative cook/wait duration bases from the runtime for runtime-backed panes and advances them locally while the runtime classification remains cooking/waiting; only fallback panes synchronize duration state from the dashboard
- when launched from tmux, defaults the selected row to the invoking pane when that pane is visible in the dashboard
- subscribes to the central runtime for shared pane state and yolo policy; reconnect bootstrap replaces the complete runtime-pane cache while transient disconnects retain the last good view
- fetches recovery offers independently from live classifier panes and leaves their PID, CPU, memory, cook, wait, YOLO, and classifier columns blank
- recovery requires an external tool to recreate the tmux server, sessions, windows, and shell panes first; botctl does not restore tmux objects or layouts
- matches only the exact original object on the same server or one exact logical pane by socket path, session name, window index/name, pane index, and cwd; cwd-only, fuzzy, ambiguous, incompatible-shell, and globally conflicting matches remain disabled
- recovery rows cover Claude, Grok, OpenCode, Antigravity, Pi, and Codex panes and preview the exact resume command (`claude --resume <uuid>`, `grok --resume <uuid>`, `opencode --session <id>`, `agy --conversation <uuid>`, `pi --session <uuid>`, or `codex resume <uuid>` after `cd <cwd> && `), original identity, lifecycle, target, or precise disabled reason
- lowercase `r` refreshes; uppercase `R` stages one selected ready `Crashed` recovery; uppercase `D` dismisses a selected `Crashed`, `Staged`, or `Uncertain` recovery; a current `Staging` recovery cannot be dismissed
- `Enter` only navigates to a live pane or uniquely matched recovery target; it never stages or submits a command
- staging pastes the exact preview without a newline or Enter; botctl does not clear shell input, and the operator must inspect the pane and press Enter manually
- `Staged` and `Uncertain` records are not pasted again automatically
- `Crashed` rows persist until the same provider session id is observed live again (`Resolved`), the operator dismisses them, or they age out three days after the crash; tmux inventory failures list them unmatched and disabled instead of hiding them
...
| --- | --- | --- |
| `ChatReady` | `submit-prompt`; `keep-going` loop submission | composer recovery commands require existing text instead |
| `PromptEditing` | `submit-composer` for existing text; `clear-composer` to destructively discard it | `submit-prompt`, because it supplies new text |
| `BusyResponding` | wait; observe with `serve`; composer recovery only with occupied text and freshly `confirmed` ghost-shell evidence | composer mutation when a tool child exists, the probe is unavailable, or other busy evidence is visible |
...
### `install-skill` / `view-skill`

Purpose: install or print the Agent Skill bundled inside the botctl binary (`botctl-prompt` by default). The skill teaches agents how to run advanced `botctl prompt` invocations.

```bash
botctl install-skill
botctl install-skill --name botctl-prompt --path ~/.claude/skills
botctl view-skill
botctl view-skill botctl-prompt
```

Default install roots are `~/.claude/skills/<name>/SKILL.md` and `~/.agents/skills/<name>/SKILL.md`. Pass `--path DIR` to write only `<DIR>/<name>/SKILL.md`. `view-skill` prints the embedded Markdown to stdout without writing files. The same skill is also available via `npx skills add colinmollenhour/botctl --skill botctl-prompt`.

### `last-message`

Purpose: dump the latest assistant message for one Claude Code, Codex CLI, OpenCode, Pi, Grok, or Antigravity pane as raw Markdown. Claude/Codex/OpenCode/Pi/Grok read persisted provider transcripts (Grok from `~/.grok/sessions/.../updates.jsonl`); Antigravity scrapes the pane's tmux scrollback.
...
Provider notes:
- Claude binds the pane to a transcript under `~/.claude/projects` using, in order: an open project transcript FD in the pane process tree, Claude's live `~/.claude/sessions/<pid>.json` registry, `--session-id` on the Claude command line, then a project-dir transcript only when it is unique for the pane cwd. Multiple same-cwd sessions without a unique binding fail closed instead of returning the newest unrelated transcript.
...
`status` and `doctor` expose the same composer and process evidence in
line-oriented and JSON output:

| Field | Values and meaning |
| --- | --- |
| `composer` | `empty`, `occupied`, or `unavailable`, derived from the visible Claude composer. |
| `ui_shells_running` | Visible shell count reported by Claude's UI: `none` or a positive count in line-oriented output, and a non-negative integer in JSON. This is screen evidence, not a process-tree result. |
| `tool_child_probe` | `present`, `confirmed-absent`, or `unavailable`, derived from the pane process tree. |
| `ghost_shell` | `confirmed` only when the UI reports a running shell and the probe confirms no tool child; `unknown` when that UI evidence exists but the probe is unavailable; `not-detected` otherwise. |
| `tool_child_probe_reason` | Machine-readable unavailability reason; `none` in line-oriented output or `null` in JSON when the probe is available. |
| `tool_child_probe_recommendation` | Probe retry action when unavailable; `none` in line-oriented output or `null` in JSON when the probe is available. |

These fields are evidence, not an automatic recovery request. Inspect the
composer before choosing `submit-composer` or the destructive
`clear-composer`; an unavailable probe never authorizes composer mutation.
...
- while no client is attached, pauses ratatui drawing, git enrichment, process resource aggregation, CPU sampling, and full row rebuilding
- while detached, keeps runtime-backed tmux window prefixes current from runtime subscription events and refreshes fallback-only provider titles on a three-second cadence
- after reattachment, completes a successful full refresh before drawing a new frame; the first CPU sample is unavailable because detached CPU baselines are discarded
...
Safety: refuses ambiguous targets, `--max-steps 0`, and every occupied composer;
it names explicit recovery commands instead of submitting or destroying existing
text. It may stop early if a permission dialog would require more than one
approval.
...
Purpose: submit the exact text already visible in a Claude composer through a
guarded, verified action.

Syntax:
```bash
botctl submit-composer --pane %ID|session:window.pane [--submit-delay-ms N]
```
...
Required state: an occupied composer in `PromptEditing`, or an occupied composer
in `BusyResponding` when the UI reports at least one running shell, no other
busy UI evidence is present, and a fresh process probe confirms that no tool
child exists (`ghost_shell=confirmed`).

Verified postcondition: success is reported as `outcome=submitted` only after
the original composer text is consumed and the pane reaches a state that
affirmatively proves submission was accepted. Botctl reloads the live submit
binding before each attempt. It makes at most three attempts, waits up to five
seconds after each attempt for the postcondition, and revalidates the exact
target, composer text, state, activity, and process evidence before retrying.
There is no `--force`.
...
Purpose: destructively discard the exact text already visible in a Claude
composer through a guarded, verified action.

Syntax:
```bash
botctl clear-composer --pane %ID|session:window.pane [--submit-delay-ms N]
```
...
Required state: the same occupied-composer guard as `submit-composer`.

Verified postcondition: success is reported as `outcome=cleared` only after the
composer is empty and the pane is `ChatReady`; a still-`BusyResponding` pane is
accepted only for the same freshly confirmed ghost-shell condition. Botctl
reloads the live clear-input binding before each of at most three attempts and
waits up to five seconds after each attempt, revalidating all guards before a
retry. There is no `--force`.
...
This command is destructive. Capture or inspect the existing text before
running it.

Common exit-2 outcomes for both composer commands:

| Outcome | Meaning |
| --- | --- |
| `guard-refused` | The composer is empty or unavailable, the state is not authorized, a live tool child makes mutation unsafe, or the required process probe is unavailable. No key is sent. |
| `target-changed` | The exact pane identity changed or disappeared. |
| `precondition-changed` | The state, cursor, activity, authorization, or equivalent pre-key guard changed. |
| `composer-changed` | Text changed, was merged, became partial, or could be foreign. Botctl retains it and never clears it. |
| `state-changed` | The composer changed state without an affirmative submit/clear postcondition. |
| `input-dead` | All three attempts left the exact composer unchanged. The residual text is retained. |

An `input-dead` refusal prints the exact alternative recovery command for the
resolved pane: failed `submit-composer` names
`botctl clear-composer --pane %ID`; failed `clear-composer` names
`botctl submit-composer --pane %ID`. For any changed-state or changed-composer
outcome, inspect with `botctl status --pane %ID` and choose a recovery again;
do not replay a stale command.

### `submit-prompt`

Purpose: resolve **new** prompt text from `--text` or `--source` and submit it
to an existing Claude pane with one ownership-safe staging transaction. It
does not submit or replace text that is already in the composer.

Syntax:
```bash
botctl submit-prompt --session NAME --pane %ID|session:window.pane \
  [--state-dir PATH] [--workspace PATH|UUID] \
  [--source PATH | --text TEXT] \
  [--when-ready [--ready-timeout-ms N]] [--submit-delay-ms N]
```
...
- `--source PATH` or `--text TEXT` (exactly one required)
- `--when-ready` (optional; synchronously and passively wait for readiness)
- `--ready-timeout-ms N` (default: `30000`, must be at least `1`, and requires `--when-ready`)
- `--submit-delay-ms N` (default: `250`, must be at least `1`)

Targeting and readiness:

- without `--when-ready`, botctl checks readiness immediately
- with `--when-ready`, botctl polls only the initially resolved exact pane until
  two stable checks show `ChatReady` with an empty composer
- the wait sends no key, pastes no text, and writes no pending-prompt record
- timeout exits `2` with `outcome=ready-timeout`, the last state, composer
  disposition, and elapsed timeout; the pane and pending store are unchanged
- target identity changes exit `2` with `outcome=target-changed`

After readiness, botctl verifies the live submit binding, pastes the new text,
waits up to 15 seconds for a complete matching composer, and then uses the same
verified submit action. Success is reported only after the composer is consumed
and the pane reaches an affirmative accepted state.

If the composer remains non-matching through the 15-second staging deadline,
botctl exits `2` with `outcome=composer-changed` and retains the text. If no
occupied composer can be proven, it exits `2` with
`outcome=staging-unverified`. If submission remains `input-dead`, the staged
text is retained so botctl cannot erase whitespace or text that another actor
may have changed. Guard, precondition, target, and state-change refusals also
exit `2`. There is no `--force`.
