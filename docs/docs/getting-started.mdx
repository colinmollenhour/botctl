...
For a one-shot prompt that launches Claude in a new tmux session, submits through the guarded TUI path, and prints only the assistant answer to stdout:
...
By default, stdout contains only the assistant answer. Pass `--verbose` to write launch/wait progress to stderr. Arguments after `--` pass through to Claude. The tmux session remains available for inspection.
...
Use `--out -` to stream the raw Markdown body to stdout instead of writing a file. The command supports Claude, Codex, OpenCode, and Pi transcript storage. Claude is resolved from `~/.claude/projects`, Codex from `~/.codex/sessions`, OpenCode from its SQLite `message`/`part` rows after the same pane title plus cwd resolution used by the dashboard, and Pi from JSONL sessions under `~/.pi/agent/sessions` (or `PI_CODING_AGENT_SESSION_DIR`). It writes assistant-visible text only; tool calls and provider metadata are not included.
