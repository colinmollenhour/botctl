...
If you just want to get value quickly, install it, start `runtime`, open `dashboard`, then try `yolo` or `serve`.
...
The runtime owns live tmux observation, pane classification, yolo supervision, and guarded actions. `dashboard`, `yolo`, and `serve` connect to the same local socket at `<state-dir>/runtime.sock`, so they all see the same pane state.
...
If you are not sure which one you want, you probably want `runtime` plus `serve`, not `observe`.
