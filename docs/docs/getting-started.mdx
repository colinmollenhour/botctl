...
- `dashboard` for a live control surface across Claude Code panes plus screen-detected Codex CLI and passive OpenCode/Pi visibility
...
If you just want to get value quickly, install it, start `runtime`, open `dashboard`, then try `yolo`, `serve`, `prompt`, or `last-message`. Everything else supports setup, diagnostics, prompt handoff, and lower-level recovery.
