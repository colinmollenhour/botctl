...
- `dashboard` for a live control surface across Claude Code panes plus screen-detected Codex CLI and passive OpenCode/Pi/Grok/Antigravity visibility, including conservative Claude/Grok recovery offers after an external tool recreates missing tmux shell panes
...
By default, stdout contains only the assistant answer. `prompt` uses the shared `botctl` tmux session unless `--session NAME` overrides it, creates that session automatically when needed, and creates one new window per run. Normal `ChatReady` plus an empty composer kills that captured window only when its captured pane remains the sole exact pane; otherwise botctl warns and retains it. A confirmed stale shell-running footer or stable residual composer text returns the answer with a warning and retains the window. Failed prompt windows remain available for inspection. Pass `--verbose` to write launch/wait progress to stderr. Arguments after `--` pass through to Claude.
...
Recovery rows are separate from live classified panes. Another tool must recreate the tmux server, sessions, windows, and shell panes first. Botctl offers recovery only for a previously verified Claude pane with a valid Claude session UUID, and only when exact pane identity or exact logical coordinates plus cwd produce one unambiguous compatible shell target. `Crashed` means the original pane object was absent from a successful stable all-pane inventory; it does not infer why the pane disappeared, and failed inventories create nothing. Press uppercase `R` to stage the displayed resume command without Enter, inspect the target, and press Enter yourself. Lowercase `r` remains refresh, uppercase `D` dismisses eligible recovery rows, and `Enter` only navigates. Botctl does not clear input, restore tmux layout, or automatically retry `Staged` or `Uncertain` records.

| Emoji | State | Meaning | Automation |
| --- | --- | --- | --- |
| `💤` | `ChatReady` | Ready for input | `submit-prompt` can run |
| `💬` | `PromptEditing` | Ready with unsubmitted input | `submit-composer` or destructive `clear-composer` |
| `🤔` | `UserQuestionPrompt` | Waiting for an operator decision or follow-up | manual review |
| `⚙️` | `BusyResponding` | Still working | wait; composer recovery requires freshly confirmed ghost-shell evidence |
