...
`prompt` is a guarded TUI workflow, not a headless `claude -p` wrapper. It needs real input, a Claude-owned pane, safe classified states, working Claude keybindings, and a fresh transcript message before it will print to stdout.
...
- Run `botctl doctor` if keybindings are missing.
