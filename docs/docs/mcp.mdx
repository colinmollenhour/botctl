...
`botctl mcp` exposes a small MCP-compatible JSON-RPC API for persistent agent TUI sessions backed by tmux. Claude is the default provider; Codex, OpenCode, and Pi are also supported on `spawn`.
...
- `spawn` — start a persistent agent TUI in a managed tmux window for an existing `cwd`. Accepts an optional `provider` (`claude` default, `codex`, `opencode`, `pi`).
