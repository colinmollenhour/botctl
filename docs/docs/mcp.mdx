...
- For Agy, fresh-message extraction is not yet supported, so `prompt` responses return `fresh_message: false` with a `stale_transcript` warning; use `snapshot` to inspect the pane.
