...
- `spawn` — start a persistent agent TUI in a managed tmux window for an existing `cwd`. Accepts optional `provider` (`claude` default, `codex`, `agy`), `model`, `effort` (`low`/`medium`/`high`/`xhigh`/`max`), `agent`, and the **claude-only** `permission_mode` (`acceptEdits`/`auto`/`bypassPermissions`/`default`/`dontAsk`/`plan`) and `settings` (a settings JSON file path or a JSON string). Per-provider support:
...
- `one_shot` — create a temporary managed session, run **exactly one** prompt to a terminal outcome, then **always attempt to kill** the window (best-effort cleanup). Required args: `cwd`, `prompt` (non-empty). Optional: `provider`/`model`/`effort`/`agent`/`permission_mode`/`settings`/`timeout_ms`/`policy` (same validation as `spawn`; `permission_mode`/`settings` are claude-only). It is implemented by composing `spawn` (with `initial_prompt`) + `kill`, so the prompt is submitted exactly once.
