...
- `one_shot` — create a temporary managed session, run **exactly one** prompt to a terminal outcome, then **always attempt to kill** the window (best-effort cleanup). Required args: `cwd`, `prompt` (non-empty). Optional: `provider`/`model`/`effort`/`agent`/`timeout_ms`/`policy` (same validation as `spawn`). It is implemented by composing `spawn` (with `initial_prompt`) + `kill`, so the prompt is submitted exactly once.
